...
- **M4 [deferred — blocked on xls-bin].** RTL-level functional verification with `simulate_module_main`. Until then, M1's DSLX `#[test]` cases + `--compare=jit` provide DSLX-vs-IR equivalence; we proceed trusting XLS's upstream-tested codegen. M4 catches up when the binary lands.
